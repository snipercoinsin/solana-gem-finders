...
class TelegramService {
  ...
  initialize() {
    ...
    if (!token || !channelId) {
      console.log('[TELEGRAM] Bot token or channel ID not configured. Telegram notifications disabled.');
      return false;
    }
  ...
  async sendNewTokenAlert(token: TokenInfo): Promise<boolean> {
    ...
    try {
      ...
      const priceChangeStr = priceChange !== null 
        ? `${priceChange >= 0 ? '+' : ''}${priceChange.toFixed(2)}%` 
        : 'N/A';
      const priceChangeIndicator = priceChange !== null ? (priceChange >= 0 ? '[UP]' : '[DOWN]') : '';

      const safetyIndicator = token.safetyScore >= 75 ? '[SAFE]' : token.safetyScore >= 50 ? '[CAUTION]' : '[RISK]';
      ...
      if (token.ownershipRenounced) message += `[OK] Mint and freeze authority renounced\n`;
      if (token.liquidityLocked) message += `[OK] 100% liquidity locked\n`;
      if (token.honeypotSafe) message += `[OK] No high-risk indicators detected\n`;
      message += `\n`;
      ...
      message += `*Links:*\n`;
      const links: string[] = [];
      if (token.dexscreenerUrl) links.push(`[Dexscreener](${token.dexscreenerUrl})`);
  ...
  async sendScanSummary(scanned: number, passed: number, failed: number): Promise<boolean> {
    if (!this.isInitialized || !this.bot || !this.channelId) {
      return false;
    }
